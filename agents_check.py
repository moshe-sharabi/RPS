    ...
    for _gamma in range(11):
        ...
        for match in histories:
            ...
            for i in range(len(match)-2):
                ...
                comp_move = agent1.predict(cur_hist).best_counter()
                game = PAIR_TRANSLATOR[played + comp_move]
                comp_result = neg[game][INDEX_OF_RESULT]
                score1 += points[comp_result]

                comp_move = agent1.predict(cur_hist).best_counter_probabilistic()
                game = PAIR_TRANSLATOR[played + comp_move]
                comp_result = neg[game][INDEX_OF_RESULT]
                score_prob1 += points[comp_result]
        ...
        print(f"Ai2_probapilistic({epoch},5,{gamma}) score: {score_prob1}, normalized: {norm1}")
        save_to.write(f"Ai2_probapilistic({epoch},5,{gamma}) score: {score_prob1}, normalized: {norm1}\n\n")
