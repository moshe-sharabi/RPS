...
save_to = open(os.path.join('.', 'agent_scores_len-25.txt'), 'w')
...
for epoch in range(5,11):
    for _gamma in range(5,9):
        arr[(epoch-5)*4 + _gamma-5] = (epoch, _gamma)
...
for epoch in range(5,11):
    for _gamma in range(5,9):
        score1 = res[(epoch-5)*4 + _gamma-5]["Ai2"]
